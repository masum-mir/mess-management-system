...
//////        String sql = "INSERT INTO meal (meal_date, meal_type,  description,  month, year, created_by) " +
//////                "VALUES (?, ?, ?, ?, ?, ?, ?)";
//////
...
////////    String sql = """
////////        INSERT INTO meal (
////////            meal_date, meal_type, cost, description,
////////            total_attendees, per_person_cost,
...
////////        throw new RuntimeException("Failed to fetch generated meal_id!");
////////    }
////////
////////    return keyHolder.getKey().intValue();
////////}
...
//////        String sql = "UPDATE meal SET meal_date = ?, meal_type = ?, description = ?, " +
//////                " month = ?, year = ? WHERE meal_id = ?";
//////
...
////
////    public List<Meal> findAll() {
////        String sql = "SELECT * FROM meal ORDER BY meal_date DESC, meal_type";
////        return jdbcTemplate.query(sql, mealRowMapper);
////    }
////
////    public List<Meal> findByMonth(int month, int year) {
////        String sql = "SELECT * FROM meal WHERE month = ? AND year = ? ORDER BY meal_date DESC, meal_type";
////        return jdbcTemplate.query(sql, mealRowMapper, month, year);
////    }
////
////    public List<Meal> findByDate(LocalDate date) {
////        String sql = "SELECT * FROM meal WHERE meal_date = ? ORDER BY meal_type";
...
////    }
////
////    public Meal findByDateAndType(LocalDate date, String mealType) {
////        String sql = "SELECT * FROM meal WHERE meal_date = ? AND meal_type = ?";
////        List<Meal> meals = jdbcTemplate.query(sql, mealRowMapper, java.sql.Date.valueOf(date), mealType);
////        return meals.isEmpty() ? null : meals.get(0);
////    }
////
////    @Transactional
...
////            ps.setDate(1, java.sql.Date.valueOf(meal.getMealDate()));
////            ps.setString(2, meal.getMealType());
////            ps.setInt(4, meal.getMonth());
////            ps.setInt(5, meal.getYear());
////            ps.setInt(6, meal.getCreatedBy());
////            return ps;
////        }, keyHolder);
////
////        return keyHolder.getKey().intValue();
...
////    }
////
////    public void updateAttendance(MealAttendance attendance) {
////        String sql = "UPDATE meal_attendance SET is_present = ?, marked_by = ? " +
////                "WHERE meal_id = ? AND member_id = ?";
////        jdbcTemplate.update(sql, attendance.getIsPresent(), attendance.getMarkedBy(),
////                attendance.getMealId(), attendance.getMemberId());
////    }
////
////    // Meal Share methods
////    private final RowMapper<MealShare> mealShareRowMapper = (rs, rowNum) -> {
...
public class MealRepository {
    ...
    public List<Meal> findByMonthAndYear(int month, int year) {
        String sql = "SELECT * FROM meal WHERE month = ? AND year = ? " +
    ...
    public Meal findByDateAndType(LocalDate date, String mealType) {
        String sql = "SELECT * FROM meal WHERE meal_date = ? AND meal_type = ?";
        List<Meal> meals = jdbcTemplate.query(sql, mealRowMapper,
                java.sql.Date.valueOf(date), mealType);
        return meals.isEmpty() ? null : meals.get(0);
    }

    // Save meal
    public Integer save(Meal meal) {
        String sql = "INSERT INTO meal (meal_date, meal_type, " +
                "total_attendances, month, year, created_by) " +
                "VALUES (?, ?, ?, ?, ?, ?)";
        ...
        jdbcTemplate.update(connection -> {
            ...
            ps.setDate(1, java.sql.Date.valueOf(meal.getMealDate()));
            ps.setString(2, meal.getMealType());
            ps.setInt(3, meal.getTotalAttendances());
            ps.setInt(4, meal.getMonth());
            ps.setInt(5, meal.getYear());
            ps.setInt(6, meal.getCreatedBy());
            return ps;
    ...
    public void updateTotalAttendances(Integer mealId) {
        String sql = "UPDATE meal SET total_attendances = " +
                "(SELECT COUNT(*) FROM meal_attendance " +
                "WHERE meal_id = ? AND is_present = TRUE) " +
                "WHERE meal_id = ?";
        jdbcTemplate.update(sql, mealId, mealId);
    }
