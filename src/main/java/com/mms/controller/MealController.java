...
//        model.addAttribute("selectedMonth", selectedMonth);
//        model.addAttribute("selectedYear", selectedYear);
//        model.addAttribute("pageTitle", "Meal Management");
//
//        return "meal/list";
//    }
//
...
//
//    /**
//     * Get meal data as JSON for edit modal
//     * URL: GET /meals/api/{id}
//     */
//    @GetMapping("/api/{id}")
//    @ResponseBody
//    public Map<String, Object> getMealData(@PathVariable Integer id) {
//        Map<String, Object> response = new HashMap<>();
//
//        try {
//            Meal meal = mealService.getMealById(id);
...
//
//            response.put("mealId", meal.getMealId());
//            response.put("mealDate", meal.getMealDate().format(dateFormatter));
//            response.put("totalAttendances", meal.getTotalAttendances());
//            response.put("memberId", meal.getMemberId());
//
//        } catch (Exception e) {
//            response.put("error", e.getMessage());
//        }
...
//        } catch (Exception e) {
//            redirectAttributes.addFlashAttribute("errorMessage",
//                    "❌ Error updating meal: " + e.getMessage());
//            return "redirect:/meals";
//        }
//    }
//
...
public class MealController {
    ...
    /**
     * Daily meal entry page
     * URL: /meals
     */
    @GetMapping
    public String dailyMealEntry(@RequestParam(required = false) Boolean success,
                                 Model model) {
        if (Boolean.TRUE.equals(success)) {
            model.addAttribute("successMessage", "✅ Meal data saved successfully!");
        }

        model.addAttribute("pageTitle", "Daily Meal Entry");
        return "meal/list";
    }

    /**
     * Get daily meal data (AJAX endpoint)
     * URL: GET /meals/daily-data?date=2024-12-16
     */
    @GetMapping("/daily-data")
    @ResponseBody
    public Map<String, Object> getDailyMealData(@RequestParam String date) {
        Map<String, Object> response = new HashMap<>();
        ...
        try {
            ...
            // Get existing meal data for this date
            List<Meal> meals = mealService.getMealsByDate(mealDate);

            // Create meal data map: memberId -> totalAttendances
            Map<Integer, Integer> mealData = new HashMap<>();
            for (Meal meal : meals) {
                mealData.put(meal.getMemberId(), meal.getTotalAttendances());
            }

            response.put("members", members);
            response.put("mealData", mealData);
    ...
    /**
     * Save daily meal data (AJAX endpoint)
     * URL: POST /meals/save-daily
     */
    @PostMapping("/save-daily")
    @ResponseBody
    public Map<String, Object> saveDailyMeals(@RequestBody Map<String, Object> requestData) {
        Map<String, Object> response = new HashMap<>();
        ...
        try {
            ...
            List<Map<String, Object>> meals = (List<Map<String, Object>>) requestData.get("meals");

            // Delete existing meals for this date first
            List<Meal> existingMeals = mealService.getMealsByDate(mealDate);
            for (Meal existing : existingMeals) {
                mealService.deleteMeal(existing.getMealId());
            }
            ...
            for (Map<String, Object> mealData : meals) {
                Meal meal = new Meal();
                meal.setMealDate(mealDate);
                meal.setMemberId((Integer) mealData.get("memberId"));
                meal.setTotalAttendances((Integer) mealData.get("totalAttendances"));
                meal.setCreatedBy(1); // Default admin user
            ...
            response.put("success", true);
            response.put("message", "Meal data saved successfully!");
    ...
    /**
     * Meal history page
     * URL: /meals/history?month=12&year=2024
     */
    @GetMapping("/history")
    public String mealHistory(@RequestParam(required = false) Integer month,
                              @RequestParam(required = false) Integer year,
                              Model model) {
        LocalDate now = LocalDate.now();
        int selectedMonth = (month != null) ? month : now.getMonthValue();
        ...
        model.addAttribute("selectedMonth", selectedMonth);
        model.addAttribute("selectedYear", selectedYear);
        model.addAttribute("pageTitle", "Meal History");
